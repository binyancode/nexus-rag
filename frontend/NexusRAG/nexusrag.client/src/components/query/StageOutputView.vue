  ...
  <div class="output-view">
    ...
    <div v-else-if="stage?.stage_id === 'initializer'" class="context-view">
      ...
      <div class="context-grid">
        ...
        <div><span>可见实体</span><b>{{ output.entity_catalog?.length || 0 }}</b></div>
      </div>
      ...
      <div v-else class="result-columns">
        <section><h4>结构化事实</h4><div v-if="!facts.length" class="empty">没有事实结果</div>
          <div class="result-list"><div v-for="(f,i) in facts" :key="f.entity_id || f.fullname || i" class="fact-item">
            ...
            <small v-if="f.entity_id">{{ f.entity_id }}</small>
          </div></div>
        </section>
        <section><h4>原文依据</h4><div v-if="!evidence.length" class="empty">没有原文依据</div>
          <div class="result-list"><div v-for="(b,i) in evidence" :key="b.fullname || i" class="evidence-item">
            <b>{{ b.title || b.fullname }}</b><small>{{ b.section }} · {{ b.fullname }}</small><p>{{ snippet(b.text) }}</p>
          </div></div>
        </section>
      </div>
...
const props=withDefaults(defineProps<{stage:QueryStageState|null;nodes?:QueryNodeState[];runAnswer?:string|null;citations?:Array<{fullname:string;quote?:string|null}>}>(),{nodes:()=>[],citations:()=>[]})
