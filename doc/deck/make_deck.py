...
DECK = [
    ...
    ("img", "02b_index_to_search", D.index_to_search, "END-TO-END", "端到端流程 · 建库 → 查询（哪里用 LLM / Embedding）", 2,
     "一图看懂端到端：上=索引阶段(离线)把文档切块→抽实体归一→建边→块向量化,写入三存储;中=两层图三存储(实体索引/边表图/块向量+原文);下=查询阶段(在线)问题→SQG编译→优化器→执行(Search精确/语义·图·集合)→回答溯源。标注:🟣LLM(抽实体/建边/归一/SQG编译/回答),🟢Embedding(块向量化/归一召回/语义查询,索引与查询同一模型),⚪无模型(切块/优化器/图遍历/集合/校验/溯源)。精确入口零模型;校验层刻意确定性不用模型。"),
    ...
    ("img", "04f_store_collection", D.store_collection, "STORES", "多实例存储 · Store / Collection 作用域", 4,
     "块可分散在多个AI Search(密钥放凭据azure_ai_search,不写config)。Store=一条AI Search凭据+注册表;Collection=store集合(多对多,一个store可属多个collection),纯查询期过滤器,一次只选一个不跨。实体只认store(经evidence),不物化实体↔collection;判定实体在不在collection= evidence.store_id∈collection.stores。查询选C→allowed_stores→Search/Traverse/Ground都按store_id过滤,指向collection外store的证据不查。重排取舍:单store collection无跨源重排,多store collection需扇出+外部reranker(用户自担)。原则:fullname/block id不带store、collection不入任何存下来的id,只当查询参数,加/移store零重算。"),
    ("img", "05_index", D.index_flow, "INDEX", "索引阶段", 5,
     "索引阶段四步：切块赋fullname→抽实体归一→LLM建边→块向量化，落入三存储。建边只用LLM(候选提名+LLM裁定)。"),
    ("img", "05b_entity_onboarding", D.entity_onboarding, "INDEX", "实体入网与增量更新", 5,
     "实体不是一次性批处理:三种来源(索引LLM抽/事前种子/事后手工)共用幂等增量算子attach_entity——归一去重→连块(evidence)→连实体(结构边),每步LLM裁定,只碰新实体邻域不全量重算。source/locked保护手工关联,系统补充关联进复核队列;文档更新按content_hash只重抽变化块与旧图diff。"),
