...
### 1.5 实体入网与增量更新（attach_entity）

实体不是"一次性批处理产物"，而是通过一个**幂等的增量算子 `attach_entity`** 落网。三种来源共用它，天生支持增量、不用全量重算：

- **索引时抽取**：LLM 从块 / 文档名 / 章节名提炼（便宜手段只做"提示信号"，不单独定实体）；
- **事前种子实体**：人工输入的已知概念清单，先进 catalog，后续抽取时 LLM 优先归一到它们；
- **事后手工新增**：用户直接调同一个 `attach_entity`。

`attach_entity(name, type?, aliases?, source)` 内部三步，每步都**局部化 + LLM 裁定**：

1. **归一 / 去重**：别名精确匹配 + 名称向量相似度召回已有候选 → LLM 判"是同一个实体吗" → 合并（并入 aliases）或新建。这就是"根据已有实体判断新名称如何入网"。
2. **连块（evidence 边）**：拿名称/别名在块存储做一次检索（向量+关键词）→ LLM 确认哪些是真正出处。代价 O(1个实体邻域)，不扫全库。
3. **连实体（结构边）**：候选邻居只来自 ② 的证据块共现 + 名称向量相近 + 同块共提 → LLM 裁定 `requires/references/belongs_to` 的类型/方向/权重。全程只动**新实体的邻域**。

**来源与保护**（实体/边新增字段）：

- 实体、边均带 `source: seed | manual | llm` 与 `locked: bool`。
- 手工关联 `locked=true`，系统**不覆盖、不删**；用户点"补充关联"时系统对该实体跑 `attach_entity` 的 ②③，产出 llm 边进**复核队列**，用户确认后落库。手工永远优先。

**增量 vs 全量**：

- 事后加实体/加边 → 只跑 `attach_entity` 的 ②③，只碰邻域，不全量重算。
- 文档更新 → 按 `content_hash` 只重抽该文档块、对受影响实体重跑 attach，并与旧图 diff（新增/失效边）。
- 只有**数据模型 / 边类型清单 / 归一策略本身变了**才全量重建。

### 1.6 Store 与 Collection（多实例存储 · 查询作用域）

块可以分散在**多个 AI Search 实例**里（密钥放在凭据里，不写 config）。为此引入两个概念：**Store**（物理存储实例）与 **Collection**（查询作用域）。

**Store（物理存储实例）**
- 一个 Store = 一条 `azure_ai_search` 凭据（endpoint + key + index_name）；注册表 `store(store_id → credential_name, index_name?, kind=block|entity)`。
- “配多个 AI Search”= 多条凭据 + 多行注册，纯数据、可增删；config 里最多留一个默认指针。

**块的身份 vs 物理位置（关键）**
- `fullname`（`类别.文档.章节.block_id`）是**逻辑身份**，用于去重/排序/溯源，**必须稳定**——**store 不进 fullname / 不进块 id**（否则迁库就会改掉所有引用）。
- 物理归属用一个 `store_id` **字段**表示，**冒余到 evidence 边上**（这样判定“某实体在不在某作用域”无需回块存储）。

**实体只认 Store，不认 Collection**
- 实体靠 evidence 连到块、块落在某 store → 实体天然“扎根”在它证据所在的 store。这层关系入库时就定了。

**Collection（查询作用域）**
- Collection = **一组 store 的集合**，与 store 是**多对多**（同一个 store 可属多个 collection）；注册表 `collection(collection_id → {store_id…})`。
- Collection **只是查询期的一层 store 过滤器**；**一次查询只选一个 collection、不跨**。
- **不物化实体/边 ↔ collection 的任何关系**；成员关系全靠推导：

  > 实体 E 在 collection C 内可答 ⟺ E 的某条 evidence 边的 `store_id ∈ C.stores`。

**查询时怎么用（纯过滤、零物化）** — 选定 C → `allowed_stores = C.stores`：
- `Search`：只打 `allowed_stores` 里的 block store；
- `Traverse / 可见性`：实体/边能不能走，用 `evidence.store_id ∈ allowed_stores` 现场判（边则要两端实体都在 C）；
- `Ground`：只取 `store_id ∈ allowed_stores` 的块。
- **指向 collection 外 store 的实体/证据一律不查。**

**重排的取舍（由 collection 里放几个 store 决定）**
- 单 store 的 collection → 语义检索只打一个 AI Search，用它内置 Semantic Ranker 就够，**无跨源重排**；
- 多 store 的 collection → 语义检索要**扇出**到多个 store，跨源候选**必须外部重排**。用户选择分散存储，就自担这个代价。

**一条原则**：store↔collection 是多对多且可动态调整，所以**绝不把 collection 烙进任何存下来的 id/字段**（块/实体/边都不带 collection），collection 永远只当**查询参数**。好处：往 collection 里加/移一个 store 只改注册表一行，**实体/块/边零重算**。
...
> 步骤 ②③ 实际上都是在调 `attach_entity`（见 §1.5）：抽实体就是对每个候选调一次 attach。因此索引管线与事后手工新增走**同一条增量路径**，只是 catalog 里有没有它的区别。

> 入库时**选一个目标 store**（见 §1.6）：块写进那个 AI Search，evidence 边上**冒余 `store_id`**。实体/边只认 store，不认 collection。
...
在线查询固定分为五个模块，另有独立的物理算子库：

1. **初始化器 `QueryInitializer`**：校验问题，确定且只确定一个当前用户可见的 Collection，冻结
  `allowed_stores`、凭据、预算和并行度，产出只读 `QueryContext`。用户未指定 Collection 时：
  唯一可见项直接选；多项时由语义路由选择；无法可靠选择则要求用户明确指定。
2. **编译器 `SQGCompiler`**：只根据问题、Collection 内可见实体词汇和逻辑算子目录生成 SQG；
  SQG 只表达「要做什么」，禁止出现数据库、向量、实体 id、边类型代码、方向、跳数等物理细节。
3. **优化器 `QueryOptimizer`**：完成实体/关系绑定、逻辑节点到物理节点展开、并行/惰性 Ground 等优化，
  产出完整 PEP。PEP 顶层显式声明生成器输入，例如
  `outputs = {facts: p5, evidence: p6}`，生成器和协调器都不猜输入。
4. **协调器 `QueryCoordinator`**：只把 PEP 转为通用 Workflow DAG 并执行，不解释问题、不改变计划。
5. **生成器 `AnswerGenerator`**：作为独立第五阶段运行，只读取 PEP 显式绑定的 facts/evidence，
  逐条返回 fullname 出处；不得自行检索。

物理算子库与协调器分离，提供 `EntitySearch / BlockSearch / Traverse / Lift / Ground /
Intersect / Diff / Union / Dedup / BlockUnion / EvidenceBundle`。Collection 是整个 query run 的安全边界：
所有 Search、实体可见性、Traverse、Lift、Ground 都必须受 `QueryContext.allowed_stores` 约束；
PEP 只能进一步缩小范围，不能扩大或更换 Collection。

初始化器同时生成当前 Collection 的真实文档目录（doc_id/title/category/store_id）。当用户明确比较多份文档时，
优化器使用每份文档独立的 `BlockSearch(doc_ids)`，再由 `EvidenceBundle` 保留原文分组；此路径不经过
`Lift / 实体集合运算 / Ground`，避免丢失文档归属。生成器按分组分别概括、比较，并校验 citation 必须属于对应组。

运行持久化分三层：`query_run` 保存总体状态、Collection/Store 快照和最终答案；`query_stage`
固定保存 initializer/compiler/optimizer/coordinator/generator 五阶段的输入、输出、状态、token、耗时和错误；
`query_node` 只保存 coordinator 运行 PEP 时的物理节点。Collection 的成员关系由 `collection_store` 保存，
公开性由 `collection.is_public` 保存，用户/角色授权及默认项由 `collection_access` 保存。
...
> 检索 / 过滤 / 重排 / 排序都在**一次 `Search` 调用**里完成；`Rerank` 默认折进 `Search` 的 `queryType=semantic`。**单 store 的 collection 无跨源重排**；只有 collection 含**多个 block store** 时，语义检索才**扇出到多 store + 外部 reranker 合并**（见 §1.6）。图遍历/多源合并的跨源候选同理。
